...
BASE_VIDEO = OUT / 'legadia_base_10s.mp4'
...
def main():
    ...
    subprocess.run([
        ...
        '-vf',
        'crop=ih*16/9:ih:(iw-ih*16/9)/2:0,'
        'scale=1280:720:flags=lanczos,'
        'minterpolate=fps=25:mi_mode=mci:mc_mode=aobmc:me_mode=bidir:vsbmc=1',
        '-t', '10', '-an',
        '-c:v', 'libx264', '-preset', 'slow', '-crf', '16',
        '-pix_fmt', 'yuv420p', str(PREP_VIDEO)
    ], check=True)

    subprocess.run([
        'ffmpeg', '-y', '-i', str(PREP_VIDEO), '-t', '8.2', '-an',
        '-c:v', 'libx264', '-preset', 'slow', '-crf', '16',
        '-pix_fmt', 'yuv420p', str(LIPS_VIDEO)
    ], check=True)
